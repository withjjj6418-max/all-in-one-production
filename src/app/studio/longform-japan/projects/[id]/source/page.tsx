...
export default function JapanLongformSourcePage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    <header><p className="text-sm font-bold text-sky-700">{projectTitle}</p><h1 className="mt-1 text-3xl font-bold">저장 원문 확인 · 수정</h1><p className="mt-2 text-sm text-muted-foreground">이 프로젝트에 이미 연결된 한국어 원문을 확인하거나 수정합니다.</p></header>
    ...
    <div className="grid gap-5 lg:grid-cols-[1.5fr_1fr]">
      ...
      <section className="rounded-2xl border border-border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-3">{sources.length === 0 && <p className="rounded-xl bg-muted p-4 text-sm text-muted-foreground">아직 저장된 원문이 없습니다.</p>}{sources.map((source) => <article key={source.id} className="rounded-xl border border-border p-4"><button onClick={() => setDraft({ id: source.id, title: source.title, sourceUrl: source.source_url || "", transcript: source.korean_transcript })} className="w-full text-left"><p className="font-bold">{source.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{source.korean_transcript}</p></button><div className="mt-3 flex flex-wrap items-center justify-between gap-2"><button type="button" onClick={() => applyProjectTitle(source.title)} disabled={savingProjectTitle || source.title === projectTitle} className="inline-flex items-center gap-1 text-xs font-bold text-brand-olive disabled:text-muted-foreground disabled:opacity-60">{savingProjectTitle ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />} {source.title === projectTitle ? "현재 프로젝트 이름" : "프로젝트 이름 변경"}</button><button onClick={() => deleteSource(source.id)} className="inline-flex items-center gap-1 text-xs font-bold text-red-600"><Trash2 size={14} /> 삭제</button></div></article>)}</div>
