...
export default function CompletedProjectsPage() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("uploaded", true) // 완료된 프로젝트만 가져옴
  ...
  const toggleCategoryCollapse = (category: string) => {
    setCollapsedCategories((prev) => {
      const currentVal = prev[category] ?? false;
      return {
        ...prev,
        [category]: !currentVal,
      };
    ...
    <div className="px-3 py-3 sm:p-5 space-y-4 max-w-7xl mx-auto min-w-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <div className="p-2 bg-green-100 text-green-700 rounded-xl shadow-inner">
            <span className="text-2xl leading-none">✅</span>
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-800 tracking-tight">완료</h1>
            <p className="text-[11px] text-gray-400 font-medium mt-0.5">업로드까지 끝난 프로젝트 모음</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm space-y-3">
          ...
          <div className="space-y-0.5">
            <h3 className="text-sm font-bold text-gray-700">아직 등록된 완료 프로젝트가 없어요</h3>
            <p className="text-xs text-gray-400 max-w-sm">프로젝트를 완료 상태로 등록하거나 가져와보세요.</p>
          </div>
          <button
            onClick={openAddModal}
